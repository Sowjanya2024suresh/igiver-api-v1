...
if(isset($_GET['ngouserid']))
{
$sql = "select pro_user_id,cus_id,pro_id,cat_id,pro_user_title,pro_user_desc,pro_user_img,crdt,dndt,status,flag,bids_opt,ngo_range,noti_status,sponsor,thanks_video_status,thanks_video_link,sku,orderdate,retail_code,qty from product_ngo WHERE cus_id IN (SELECT cus_userId from customer WHERE flag = 1) and cus_id='".$_GET['ngouserid']."'"; 
}
else
{
...
while($row = mysqli_fetch_array($res)){
    ...
    'sponsor'=>$row[14],'thanks_video_status'=>$row[15],'thanks_video_link'=>$row[16],'sku'=>$row[17],'orderdate'=>$row[18],'retail_code'=>$row[19],'qty'=>$row[20],'retail_name'=>$RetailerName,'website'=>$RetailerWebUrl,'ngo_link'=>$NGOWebUrl);
