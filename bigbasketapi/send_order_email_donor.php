    ...
    border-radius: 8px;" >
       ...
       }else  if($towhom=="ADMIN")
       {
           // $tablecontent='';
            ...
            $headmsg='New Transaction for Big Basket Wallet from iGiver.Org ';
       }
   ...
   // echo $orderid;
     ...
     if($resorderdetail)
    {
    	while($row = mysqli_fetch_array($resorderdetail))
    	{
            ...
            // exit;
            $orderdetailarray[] =array('Order Id'=>$row[0],'Description'=>$orderdesc,'Amount'=>$row[4],'NGO ID'=>$row[2],'Donated Date'=>$row[5],'Transfer Status'=>$row[6]==1?'Success':'Fail','Payment Ref No'=>$row[12]);
        ...
        if($resNGO)
        {
            ...
            {       //NGO_NAME,NGO_CONTACT_NAME,NGO_ADDRESS,NGO_MOBILE
            		...
            		$ngoname=$row[2];
            		$ngoemail=$row[3];
                    $NgoDetailArray[] =array('Igiver NGO Id'=>$row[1],'NGO Name'=>$row[2],'Email'=>$row[3],'Phone'=>$row[4],'Website'=>$row[12],'Contact Name'=>$row[15],'Address'=>$row[16]);
            }
