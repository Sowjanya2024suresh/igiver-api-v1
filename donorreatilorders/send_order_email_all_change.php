<?php
	...
	function send_email($toemail,$towhom,$orderid,$orderdetailarray,$NgoDetailArray,$DonorDetailArray,$ngoname,$NGOid)
    {
    ...
    border-radius: 8px;" >
       ...
       if($towhom=="DONOR")
       {
           ...
           $successmessage=' <div class="h3-white-center" style="color: #ff5c6d;
                                                ...
                                                text-align: center;
                                                font-weight:600;">
                                                <multiline>Thank you for your Kindness	</multiline>
                                             </div>
                                             ...
                                             <div class="text-white-center" style="color:#000; font-family:Arial,sans-serif; font-size:12px; line-height:20px; text-align:center;padding: 0 8%;">
                                               <img src="https://lokas.in/Igiver-nl/images/happy.png"></multiline>
                                             </div>
                                             <table width="100%" border="0" cellspacing="0" cellpadding="0" class="spacer" style="font-size:0pt; line-height:0pt; text-align:center; width:100%; min-width:100%">
                                                <tr>
       ...
       else if($towhom=="RETAILER")
       {
           ...
           $headmsg='Order Paid from iGiver.Org. NGO Code is '.$NGOid;
       }else if($towhom=="NGO")
       {
           $tablecontent='';
            $successmessage='<div class="text-white-center" style="color:#000; font-family:Arial,sans-serif; font-size:12px; line-height:20px; text-align:center;padding: 0 8%;">
            ...
            $headmsg='You got Donation from iGiver.Org. NGO Code is '.$NGOid;
       }
// 		require("../PHPMailer/class.phpmailer.php");
		...
		$mail->AddAddress($toemail); // recipients email
	    	//$mail->AddCC('order.info@igiver.org'); // recipients email
                //$mail->AddCC('srividhya@aramporul.in');
                //$mail->AddCC('periammact.srinath@gmail.com');
                //$mail->AddCC('sasi@lokas.info');
		$mail->From = 'info@igiver.org';
		$mail->FromName = 'iGiver.Org Order Support Team'; // readable name
		$mail->Subject =$headmsg." Order id ".$orderid;  
		$message=$headmsg.". Order id ".$orderid;
...
$url = 'http://65.0.244.240:3001/jspdf';
$htmlfile=htmlspecialchars($body);
...
echo $donoremail;
echo $toemailretailer;
echo $ngoemail;

$donoremail='sowjanya@lokas.in';
$toemailretailer='sowjanya@lokas.in';
$ngoemail='sowjanya@lokas.in';

       send_email($donoremail,"DONOR",$orderid,json_encode($orderdetailarray),json_encode($NgoDetailArray),json_encode($DonorDetailArray),$ngoname,$NGOid);
      send_email($toemailretailer,"RETAILER",$orderid,json_encode($orderdetailarray),json_encode($NgoDetailArray),json_encode($DonorDetailArray),$ngoname,$NGOid);
     send_email($ngoemail,"NGO",$orderid,json_encode($orderdetailarray),json_encode($NgoDetailArray),json_encode($DonorDetailArray),$ngoname,$NGOid);
