    ...
    else //Proceed with insert
    {
        ...
        $updatedate=date('y-m-d');
         $productid=$_GET['productid'];
          $wishtreeid=$_GET['wishtreeid'];
        
        $sqlInsert ="INSERT INTO `donor_retail_orders`( `retail_code`,`orderid`, `order_desc`, `order_amount`, `order_qty`, `order_sku`, `ngoid`, `donorid`, `orderdate`, `order_source`, `overall_status`, `paystatus`, `pay_ref_no`, `delivery_ref_no`, `flag`, `thanks_video_status`, `thanks_video_link`, `createdate`, `updatedate`, `product_id`, `wishtreeid`) VALUES ('$retail_code','$orderid',
        '$order_desc','$order_amount','$order_qty','$order_sku','$ngoid','$donorid','$orderdate','$order_source','$overall_status','$paystatus','$pay_ref_no','$delivery_ref_no','$flag','$thanks_video_status','$thanks_video_link','$createdate','$updatedate','$productid','$wishtreeid')";
       ...
       //  echo $sqlInsert;
        ...
        if($respayway)
        {
        	...
        	if($payway='INST')
        	{
        	     $im_orderid=get_imjo_create_order_web($orderid,$retail_code,$con);
        	    // echo "imorderid===".$im_orderid;
        	     $longurl=getlongurl($im_orderid,$retail_code,$con);
        	     
                //   $myArray = array(json_decode($im_orderid, true));
                //   $iorderid = $myArray[0]["order_id"];
                //   $longurl=getlongurl($iorderid,$retail_code,$con);
        ...
        // echo json_encode($orderid);
         ...
         $order_info[] =array('order_id'=>$orderid,'longurl'=>$longurl);
